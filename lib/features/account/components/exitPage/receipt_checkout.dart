...
import 'package:populargo/models/ProductScan/scannedProduct.dart';
...
class ReceiptCheckoutPage extends StatefulWidget {
  static const String routeName = '/receiptCheckoutPage';
  const ReceiptCheckoutPage({super.key});

  @override
  State<ReceiptCheckoutPage> createState() => _ReceiptCheckoutPageState();
}

// once get into this page, delete the scannedProduct data with the scannedProduct_rfid_id

// check with the user balance first, if not enough, the button would be Please Reload,
// --> then after pressed, will go to the bottombar page, delete the scannedProduct from database
// --> send a wrong sound to the exitScanner

// if enough balance, the button would be Checkout
// once chcekout is pressed,
// --> record the payments,
// --> transaction history,
// --> update userBalance,
// --> userVisit end_dateTime and duration [calculate duration],
// --> delete the wishlist based on the quantity and book_id)
//- send notification (amount paid, and time and date)
// then go to payment successful page

// --> for with discount, think about what if the user press the back button between discount page and
// receipt checkout page (when do you delete the rfid_id scanned product?)

class _ReceiptCheckoutPageState extends State<ReceiptCheckoutPage> {
  ...
  Future<void> updateExitWishlistItem(
      String user_id, List<String> bookIdList, List<int> quantityList) async {
...
// if enough balance, the button would be Checkout
// once chcekout is pressed,
// --> record the payments, --> required userVisit_id, bookIdList, quantityList, priceList, subtotal,
// estimatedTax, total, status = 'paid' [done]
// --> transaction history, --> user_id, transaction_datetime, amount, transactionType = 'Pay' [done]
// --> update userBalance, --> user_id, totalBalance [done]
// --> userVisit end_dateTime and duration [calculate duration], [done]
// --> delete the wishlist based on the quantity and book_id)
//- send notification (amount paid, and time and date)
// then go to payment successful page
